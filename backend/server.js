...
/* =====================================================
   CORS & CARTELLA PUBLIC (FRONTEND)
===================================================== */

app.use(cors());

// Serviamo i file statici solo dalla cartella public
...
/* =====================================================
   ROTTA PROTETTA DASHBOARD (ACCESSO SOLO PER UTENTI PAGANTI)
===================================================== */
...
app.get("/dashboard", (req, res) => {
  ...
  if (!email) {
    return res.redirect("/checkout.html?error=missing_email");
  }
  ...
  db.get(
    ...
    [email],
    (error, row) => {
      if (error || !row || row.premium !== 1) {
        console.log("ACCESSO NEGATO ALLA DASHBOARD PER:", email);
        return res.redirect("/checkout.html?error=unauthorized");
      }

      // L'utente risulta pagante nel DB -> Serviamo il file protetto
      res.sendFile(path.join(__dirname, "protected", "dashboard.html"));
    }
  );
});

// Reindirizza eventuali tentativi diretti a /dashboard.html
...
app.listen(PORT, "0.0.0.0", () => {
  console.log("SaaS SERVER ON PORT " + PORT);
});